...
void _Encode(DedupData& data, int fd, size_t filesize, void* buffer, tp& begin, tp& end) {
//    alignas(FIFOPlus<chunk_t*>) unsigned char* refine_input;
//    alignas(FIFOPlus<chunk_t*>) unsigned char* deduplicate_input;
//    alignas(FIFOPlus<chunk_t*>) unsigned char* compress_input;
//    alignas(FIFOPlus<chunk_t*>) unsigned char* reorder_input;
//
...
//
//    std::set<int> srefine, sdeduplicate, scompress, sreorder;
//    compute_fifo_ids_for_layer(srefine, fragment);
//    compute_fifo_ids_for_layer(sdeduplicate, refine);
//    compute_fifo_ids_for_layer(scompress, deduplicate);
//    compute_fifo_ids_for_reorder(sreorder, deduplicate, compress);
//
//    auto allocate_fifos = [](unsigned char** base, std::set<int> const& ids) {
//        *base = (unsigned char*)malloc(ids.size() * sizeof(FIFOPlus<chunk_t*>));
//    };
//
//    allocate_fifos(&refine_input, srefine);
//    allocate_fifos(&deduplicate_input, sdeduplicate);
//    allocate_fifos(&compress_input, scompress);
//    allocate_fifos(&reorder_input, sreorder);
//
//    std::map<void*, PThreadThreadIdentifier*> identifiers;
//
//    auto init_fifo = [&identifiers](void* ptr, FIFOReconfigure reconfiguration, unsigned int nb_producers, unsigned int nb_consumers, unsigned int history_size, std::string&& description) {
//        PThreadThreadIdentifier* identifier = new PThreadThreadIdentifier;
//        identifiers[ptr] = identifier;
//
//#ifdef FIFO_PLUS_TIMESTAMP_DATA
//        new(ptr) FIFOPlus<chunk_t*>(FIFOPlusPolicy::POP_WAIT, reconfiguration, identifier, nb_producers, nb_consumers, history_size, std::move(description), Globals::start_time);
//#else
//        new(ptr) FIFOPlus<chunk_t*>(FIFOPlusPolicy::POP_WAIT, reconfiguration, identifier, nb_producers, nb_consumers, history_size, std::move(description));
//#endif
//    };
//
//    FIFOPlus<chunk_t*>* refine_fifos = (FIFOPlus<chunk_t*>*)refine_input;
//    FIFOPlus<chunk_t*>* deduplicate_fifos = (FIFOPlus<chunk_t*>*)deduplicate_input;
//    FIFOPlus<chunk_t*>* compress_fifos = (FIFOPlus<chunk_t*>*)compress_input;
//    FIFOPlus<chunk_t*>* reorder_fifos = (FIFOPlus<chunk_t*>*)reorder_input;
//
//    std::map<int, FIFOPlus<chunk_t*>*> ids_to_fifos;
//
//    auto init_fifos = [&data](FIFOPlus<chunk_t*>* fifos, std::string&& description, std::set<int> const& fifo_ids, LayerData const& layer, LayerData const& dst_layer) {
//        auto nb_consumers = [&dst_layer](int fifo_id) {
//            unsigned int nb_consumers = 0;
//            for (ThreadData const& thread_data: dst_layer._thread_data) {
//                if (thread_data._inputs.find(fifo_id) != thread_data._inputs.end()) {
//                    nb_consumers++;
//                }
//            }
//
...
//            unsigned int nb_producers = nb_producers_for_fifo(*iter, layer);
//            init_fifo(fifos + i, data._algorithm, nb_producers, nb_consumers(*iter), data._fifo_data[*iter]._history_size, std::move(description));
//            ids_to_fifos[*iter] = fifos + i;
//        }
//    };
//
//    init_fifos(refine_fifos, "fragment to refine", srefine, fragment, refine);
//    init_fifos(deduplicate_fifos, "refine to deduplicate", sdeduplicate, refine, deduplicate);
//    init_fifos(compress_fifos, "deduplicate to compress", scompress, deduplicate, compress);
//
//    {
//        auto nb_consumers = [](int fifo_id) {
//            unsigned int nb_consumers = 0;
//            for (ThreadData const& thread_data: reorder._thread_data) {
//                if (thread_data._inputs.find(fifo_id) != thread_data._inputs.end()) {
//                    ++nb_consumers;
//                }
//            }
...
//            unsigned int nb_producers = nb_producers_for_reorder_fifo(*iter, deduplicate, compress);
//            init_fifo(reorder_fifos + i, data._algorithm, nb_producers, nb_consumers(*iter), data._fifo_data[*iter]._history_size, "dedup / compress to reorder");
//            ids_to_fifos[*iter] = reorder_fifos + i;
//        }
//    }
//
...
//
//    auto launch_stage = [&data, &ids_to_fifos, &barrier](LayerData const& layer, void* (*routine)(void*), ) {
//        launch_stage_args* launch_args = new launch_stage_args[layer.get_total_threads()];
//        pthread_t* threads = new pthread_t[layer.get_total_threads()];
//
//        int i = 0;
//        for (ThreadData const& thread_data: layer._thread_data) {
//            launch_stage_args* this_args = launch_args + i;
//            this_args->_start_routine = routine;
//            this_args->_barrier = &barrier;
//            ++i;
//        }
//    };
}
