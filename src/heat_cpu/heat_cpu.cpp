...
void heat_cpu(Matrix4D& array, size_t m) {
    namespace g = Globals;
    ...
    for (int j = 1; j < g::HeatCPU::DIM_Y; ++j) {
        for (int i = 1; i < g::HeatCPU::DIM_X; ++i) {
            for (int k = 0; k < g::HeatCPU::DIM_Z; ++k) {
                HeatCPUMatrix::update_matrix(array, m, i, j, k);
            }
        }
    }
}
...
void heat_cpu_promise_plus(Matrix4D& array, size_t m, PromisePlusStore& dst, const PromisePlusStore& src) {
    namespace g = Globals;
...
void heat_cpu_array_of_promises(Matrix4D& array, size_t m, 
...
void heat_cpu_promise_of_array(Matrix4D& array, size_t m, 
