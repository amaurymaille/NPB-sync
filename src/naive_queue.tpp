...
template<typename T>
Observer<T>::~Observer() {
    std::cout << "Found best step = " << _best_step << ", push cost = " << _data._cost_p << ", worst average Wi = " << _worst_avg << ", sync cost = " << _data._cost_s << std::endl;
}
...
void Observer<T>::trigger_reconfigure(bool first) {
    ...
    if (first) {
        if (!_reconfigured && std::all_of(_times.begin(), _times.end(), [this](auto const& value) {
            ...
            unsigned int best_step = std::sqrt((_data._iter * _data._cost_s) / (_data._cost_p * _times.size() + _data._wi));
...
void Observer<T>::add_cost_p_cost_s_time(NaiveQueueImpl<T>* producer, uint64_t push_time, uint64_t sync_time) {
    ...
    if (data._n_push == _cost_p_cost_s_size && data._n_push == _cost_p_cost_s_size) {
        trigger_reconfigure(true);
    }
}
...
template<typename T>
bool Observer<T>::add_cost_s_time(NaiveQueueImpl<T>* producer, uint64_t sync_time) {
    if (producer->was_reconfigured()) {
        ...
        auto time = sync_time - producer->get_step() * _data._cost_p;
        ...
        _cost_s[producer].push_back(time);
